...
def generate_comparison_report_multi(
    ...
    """Generate comparison report for any number of backends (2 or 3+).

    Supports all combinations: vllm vs sglang, vllm vs unsloth,
    sglang vs unsloth, or all three.
    """
    os.makedirs(output_dir, exist_ok=True)
    ...
    # If we have exactly vllm and sglang, use the original 2-way report
    if set(runs.keys()) == {"vllm", "sglang"}:
        return generate_comparison_report(runs["vllm"], runs["sglang"], output_dir)

    # Multi-backend comparison
    backend_names = sorted(runs.keys())
    name_map = {"vllm": "vLLM", "sglang": "SGLang", "unsloth": "Unsloth"}
