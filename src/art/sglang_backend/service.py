...
class SGLangMegatronService:
    ...
    async def train(
        ...
        _slept = False
        # Memory release tags depend on GPU overlap:
        #
        # NO OVERLAP (e.g. sglang=GPU0, training=GPU2,3):
        #   Only release weights + cuda_graph. Do NOT release kv_cache because:
        #   1. KV cache is on SGLang's GPU which isn't used for training — zero benefit
        #   2. SGLang's release_memory_occupation moves token pool tensors to CPU but
        #      the scheduler can still fire Triton kernels that expect GPU tensors,
        #      causing: "ValueError: Pointer argument (at 0) cannot be accessed from
        #      Triton (cpu tensor?)" → SIGQUIT → server death
        #   3. The KV cache will be invalidated anyway by update_weights_from_disk
        #      after training (new weights → stale KV entries), so it rebuilds regardless
        #
        # OVERLAP (same GPU for inference + training):
        #   Must free ALL memory. Use restart as fallback if release crashes.
        ...
        if strategy == "sleep_wake":
            ...
            if gpus_overlap:
                ...
                # NOTE: releasing kv_cache can crash the server on MoE models
                # (Triton kernel gets cpu tensor). If it fails, fall back to restart.
                release_ok = await self._native_release_memory(tags=_native_sleep_tags_full)
                if release_ok:
                    # Verify server is still alive after releasing kv_cache
                    await asyncio.sleep(0.5)
                    if self._server_process and self._server_process.poll() is not None:
                        print(f"  ⚠️ Server crashed after releasing kv_cache (MoE bug), falling back to restart")
                        release_ok = False
                if release_ok:
                    _slept = True
                ...
                # Separate GPUs: only release weights + CUDA graphs.
                # Do NOT release kv_cache — it's on SGLang's GPU (not used for
                # training) and releasing it crashes MoE models (Triton cpu tensor bug).
                release_ok = await self._native_release_memory(tags=_native_sleep_tags_safe)
                if release_ok:
                    _slept = True
                    print(f"  ✓ SGLang weights+graphs released (KV cache kept — separate GPU, no benefit to freeing)")
                else:
        ...
        num_lines = 0
        log_found = False
        _server_died_during_training = False
        _server_check_interval = 50  # Check every 50 iterations (~5s)
        _server_check_counter = 0
        print(f"[SGLang] Waiting for training, reading from: {log_file_path}", flush=True)
        while True:
            await asyncio.sleep(0.1)
            
            # Periodically check if SGLang server process is still alive
            # (only relevant for sleep_wake/freeze where server should survive)
            _server_check_counter += 1
            if (not _server_died_during_training 
                    and _server_check_counter % _server_check_interval == 0
                    and strategy in ("sleep_wake", "sleep", "freeze")
                    and self._server_process is not None):
                rc = self._server_process.poll()
                if rc is not None:
                    _server_died_during_training = True
                    _sglang_log = os.path.join(self.output_dir, "logs", "sglang.log")
                    _tail = ""
                    try:
                        with open(_sglang_log, "r") as _lf:
                            _tail = "".join(_lf.readlines()[-30:])
                    except Exception:
                        _tail = "(could not read log)"
                    print(f"[SGLang] ⚠️ SERVER PROCESS DIED during training (rc={rc})!", flush=True)
                    print(f"[SGLang] Last 30 lines of server log:\n{_tail}", flush=True)
                    print(f"[SGLang] Training continues — will do full restart after.", flush=True)
        ...
        if strategy in ("sleep_wake", "sleep", "freeze"):
            ...
            # wasting time on update_weights_from_disk which will just fail.
            if _server_died_during_training or (
                self._server_process is not None and self._server_process.poll() is not None
            ):
                _rc = self._server_process.returncode if self._server_process else "?"
                if not _server_died_during_training:
                    # Detected now (missed during training loop)
                    _sglang_log = os.path.join(self.output_dir, "logs", "sglang.log")
                    _tail = ""
                    try:
                        with open(_sglang_log, "r") as _lf:
                        ...
                        _tail = "(could not read log)"
                    print(f"[SGLang] ⚠️ Server process died during training (rc={_rc})", flush=True)
                    print(f"[SGLang] Last 30 lines of server log:\n{_tail}", flush=True)
                print(f"[SGLang] ⚠️ Server dead → skipping update_weights_from_disk, doing full restart", flush=True)
                self._server_process = None
                await self._stop_sglang_server()
                gc_and_empty_cuda_cache()
                await self._start_sglang_server(new_checkpoint_dir)
                await self._hot_reload_lora(new_checkpoint_dir, next_step)
                _reload_method = f"{strategy}_server_died→restart"
            else:
                # Server process is alive — proceed with normal resume
                try:
                    if _slept and strategy == "sleep_wake":
                        # ──────────────────────────────────────────────────────
                        # SLEEP_WAKE resume: ALL memory was released (weights +
                        # KV cache + CUDA graphs) via release_memory_occupation.
                        ...
                        #   1. update_weights_from_disk → reload ALL weights
                        #   2. Wait for server to become healthy
                        #   3. load_lora_adapter → apply new LoRA checkpoint
                        #
                        # NOTE: KV cache / radix cache is REBUILT from scratch
                        # after weight reload. Old KV entries were computed with
                        # old weights and are invalid.
                        # ──────────────────────────────────────────────────────
