...
def tokenize_trajectory_groups(
    ...
    skip_advantage_filter: bool = False,
    ...
    for group in trajectory_groups:
        ...
        for trajectory in group:
            ...
            # Skip trajectories with no advantage (unless skip_advantage_filter is set)
            if advantage == 0 and not skip_advantage_filter:
                continue
            # If skipping filter and advantage is 0, use small positive value for training
            if advantage == 0 and skip_advantage_filter:
                advantage = 1e-6
...
def tokenize_trajectory(
    ...
    original_token_ids = list(tokenizer.apply_chat_template(
        cast(list[dict], messages),
        tools=tools,
        continue_final_message=True,
    ))
    # Use a unique ASCII string as sentinel that will survive chat template processing
    # This is more robust than using a single unused token ID which may tokenize differently in context
    sentinal_token = "<<<SENTINEL_MARKER_7f3a9b>>>"
    sentinal_encoded = tokenizer.encode(sentinal_token, add_special_tokens=False)
    sentinal_token_id = sentinal_encoded[0] if len(sentinal_encoded) == 1 else None
    ...
    for message in messages_and_choices:
        if isinstance(message, dict):
            ...
            continue
        # Find sentinel sequence in token_ids
        start = None
        for i in range(len(token_ids) - len(sentinal_encoded) + 1):
            if token_ids[i:i + len(sentinal_encoded)] == sentinal_encoded:
                start = i
                end = i + len(sentinal_encoded)
                break
        if start is None:
            raise ValueError(
                f"Sentinel marker '{sentinal_token}' not found in token_ids. "
                f"Expected tokens: {sentinal_encoded}. "
                f"This may indicate a tokenizer incompatibility with this model."
            )
        try:
